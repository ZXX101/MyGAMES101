...
Eigen::Matrix4f get_model_matrix(float rotation_angle)
{
    ...
    model << std::cos(deg), -std::sin(deg), 0, 0,
...
{
    Eigen::Matrix4f model = Eigen::Matrix4f::Identity();
    float deg = angle * MY_PI / 180.0f;
    Eigen::Matrix3f N = Eigen::Matrix3f::Zero();
    N << 0, -axis.z(), axis.y(),
        axis.z(), 0, -axis.x(),
        -axis.y(), axis.x(), 0;
    Eigen::Matrix3f rod = std::cos(deg) * Eigen::Matrix3f::Identity() + (1 - std::cos(deg)) * axis * axis.transpose() + std::sin(deg) * N;
